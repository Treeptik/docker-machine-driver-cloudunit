...
version := "v0.0.4"
version_description := "Docker Machine Driver Plugin to access CloudUnit"
human_name := "CloudUnit Driver"

mkfile_path := $(abspath $(lastword $(MAKEFILE_LIST)))
current_dir := $(notdir $(patsubst %/,%,$(dir $(mkfile_path))))
github_user := "Treeptik"
...
compile:
	GOGC=off CGOENABLED=0 go build -ldflags "-s" -o bin/$(current_dir)$(BIN_SUFFIX) main.go
...
install:
	cp bin/docker-machine-driver-cloudunit_darwin-amd64 /usr/local/bin/$(current_dir)
